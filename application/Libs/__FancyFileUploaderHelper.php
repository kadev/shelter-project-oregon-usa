<?php
	...
	class FancyFileUploaderHelper
	{
		...
		public static function NormalizeFiles($key, $FILES)
		{
			$result = array();
			if (isset($FILES) && is_array($FILES) && isset($FILES[$key]) && is_array($FILES[$key]))
			{
				$currfiles = $FILES[$key];
		...
		public static function HandleUpload($filekey, $options = array(), $REQUEST, $POST, $FILES)
		{
			//if (!isset($REQUEST["fileuploader"]) && !isset($POST["fileuploader"]))  return false;
			...
			$files = self::NormalizeFiles($filekey, $FILES);
